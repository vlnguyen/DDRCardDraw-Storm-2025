...
function getPoolPlayersResults(poolPlayers: PoolPlayer[]): PoolPlayerResult[] {
  ...
  const poolPlayersResults: PoolPlayerResult[] = poolPlayers.map(poolPlayer => {
    const scoresSum = poolPlayer.scores.reduce((sum, score) => sum + score, 0)
    return {
      ...poolPlayer,
  ...
  for (let songIndex = 0; songIndex < numSongs; songIndex++) {
    ...
    const allSongScores = poolPlayers.reduce((prevScores, currentPoolPlayer) => {
  ...
  for (let poolPlayerResultIndex = 0; poolPlayerResultIndex < poolPlayersResults.length; poolPlayerResultIndex++) {
    ...
    if (currentWinTarget === null) {
      ...
      const prevPoolPlayerResult = poolPlayersResults[poolPlayerResultIndex - 1]
        ...
        {poolPlayersResults.map(({ playerName, scores, wins, rank, averageEx, isEliminated }, poolPlayerResultIndex) => {
          ...
          return (
            <tr key={poolPlayerResultIndex}>
              <td>
                {medal} <b>{rank}. {playerName}</b>
              </td>
              {scores.map((score, scoreIndex) => (
                <td key={scoreIndex}>{getDisplayScore(score)}</td>
              ))}
              <td><b>{wins} wins</b> (<i>Avg. {getDisplayScore(averageEx)}</i>)</td>
            </tr>
          )
        })}
      </tbody>
